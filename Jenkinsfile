...
pipeline {
    ...
    environment {
        ...
        EC2_INSTANCE_IP = "placeholder"  // Placeholder for now
        SSH_USER = "ec2-user"
        SSH_KEY_FILE = credentials('ec2-ssh-key')
    }
    ...
    stages {
        ...
        stage('Deploy to EC2') {
            steps {
                script {
                    echo "Deploying to EC2 with ECR Registry URL: ${ECR_REGISTRY_URL}, TAG: ${TAG}, SSH_USER: ${SSH_USER}, SSH_KEY_FILE: ****, EC2_INSTANCE_IP: ${EC2_INSTANCE_IP}"
                    
                    // Check if critical variables are null or undefined
                    if (!SSH_KEY_FILE || !SSH_USER || !EC2_INSTANCE_IP || !ECR_REGISTRY_URL || !TAG) {
                    ...
                    withCredentials([file(credentialsId: 'ec2-ssh-key', variable: 'SSH_KEY_FILE')]) {
                        def remoteCommand = """
                            aws ecr get-login-password --region ${AWS_DEFAULT_REGION} | sudo docker login --username AWS --password-stdin ${ECR_REGISTRY_URL}:${TAG} &&
                            sudo docker pull ${ECR_REGISTRY_URL}:${TAG} &&
                            sudo docker run -d -p 8100:8100 ${ECR_REGISTRY_URL}:${TAG}
                        """
                        
                        echo "Executing SSH command:"
                        echo "ssh -i ${SSH_KEY_FILE} -o StrictHostKeyChecking=no ${SSH_USER}@${EC2_INSTANCE_IP} '${remoteCommand}'"
                        
                        // Execute SSH command
                        sh """
                            ssh -i ${SSH_KEY_FILE} -o StrictHostKeyChecking=no ${SSH_USER}@${EC2_INSTANCE_IP} '${remoteCommand}'
                        """
